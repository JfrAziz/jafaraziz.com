...
In our [previous post](./rest-api-with-r-part-1), we demonstrated how to create a custom logging and error handling system for a basic Rplumber app. In this post, we will delve into setting up a routing system and request validation. If you wanna check another part, please refer to the links below

 - [Part 1 : Project Setup, Logging, and Error Handling](./rest-api-with-r-part-1) 
...
Now to create a new routes, just create files in `routes` dir with rplumber annotation. For more advance routing, like dynamic routing you can read it in [the docs](https://www.rplumber.io/articles/routing-and-input.html).
...
In this part, we have discussed about how to setup a file based like routing with Rplumber and add some request validation. In the next part, we will discussed about deploying Rplumber with Docker. Here the link to continue to the [next part](./rest-api-with-r-part-3).