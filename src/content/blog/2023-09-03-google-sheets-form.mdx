---
title: "What you need is Google Sheets: Submit data to Google Sheets with an API"
...
function doPost(e) {
  ...
  try {
     ...
     * header must be in the first row and firs column
     * */
    const headers = sheet
    ...
    const newRow = headers.map(function (header) {
      switch (header) {
        case "no":
          return nextRow - 1;
        casinsert the data to last rowe "date":
          return new Date();
        default:
